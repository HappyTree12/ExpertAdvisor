...
extern double AverageTrueRange = 14;  // Replace ATR with TP and SL for MACD_EMA
extern int CooldownDuration = 60;     // Cooldown period in seconds
extern int Choice = 0;                // Strategy selection (1: MACD_EMA, 2: MA7, 3: Both)
...
void PlaceOrderMACD(int orderType, double price, string comment)
  {
   ...
   double roundedATR = MathFloor(atrValue * 100) / 100.0;
  
   
   double sl = (orderType == OP_BUY) ? price - (roundedATR * 0.5) : price + (roundedATR * 0.5);
   double tp = (orderType == OP_BUY) ? price + (roundedATR * 1.0) : price - (roundedATR * 1.0);
